...
public struct SecureProjectFileWriter: Sendable {
  ...
  public func readContent(
    ...
    #if os(Windows)
      return try windowsContent(relativePath: relativePath, resolver: resolver)
    #else
      let rootFD = open(
        resolver.root.canonicalPath,
        O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW
    ...
    private func stagingDescriptor(parentFD: Int32, name: String) throws -> Int32 {
      for _ in 0..<8 {
        let candidate =
          name.isEmpty ? ".codexbridge.staging.\(UUID().uuidString.lowercased())" : name
    ...
    private func windowsRevision(
      ...
      guard let data = try windowsContent(relativePath: relativePath, resolver: resolver) else {
        return nil
      }
      return .digest(of: data)
    }
