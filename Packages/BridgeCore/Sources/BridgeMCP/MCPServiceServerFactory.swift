...
public struct MCPServiceServerFactory: Sendable {
  ...
  package static func instructions(customInstructions: String) -> String {
    let base =
      ...
      + "omit execution_model and execution_effort unless "
      ...
      + "provider_session_id returned by get_task as thread_id. skill_name must be omitted. OpenCode execution_effort is optional and must match "
      + "a value advertised for the selected model. DeepSeek Harness must be selected explicitly "
      + "with provider_id=deepseek-harness and a verified installation; it supports "
      + "provider-native read-only or workspace-write modes. To continue a completed session, pass "
      + "its provider_session_id as thread_id when list_agents reports lifecycle.session_continue. Its optional execution_model and execution_effort "
      ...
      + "is injected by Bridge. Network and sandboxed "
