...
Evidence checked on 2026-08-17 against official repositories and releases. Bridge-controlled Swift dependencies remain exact and `Package.resolved` is committed because the critical dependencies are pre-1.0 or infrastructure-sensitive. External Agent providers follow their own compatible release cadence and are validated at registration and Probe time.

| Dependency | Version / selection | Product | License | Project boundary | Primary evidence |
|---|---:|---|---|---|---|
...
| DeepSeek Harness | user-built official source at a compatible revision (this guide checked `dsh-v0.1.5-rc.2`) | external ACP executable | upstream project terms | `BridgeDeepSeekHarnessACP` process boundary; Bridge validates entry and artifact identity, bundles the `cordis.yml` model/effort template, and never bundles Harness or reads its `.env` | [DeepSeek Harness guide](./DEEPSEEK_HARNESS_CONNECTION_GUIDE.md) |
...
- DeepSeek Harness is not downloaded or bundled. The user builds an official source revision, selects `apps/cli/lib/bin.js` for the modern profile-based ACP entry (the older `packages/examples/acp-demo/lib/bin.js` remains compatible), and provides an external profile. Bridge validates source/runtime/config artifact identity; ACP wire protocol 1 and the Node minimum remain the runtime boundary, while DSH package versions are not exact Bridge pins. The modern launch uses `--profile acp --patch` with a private per-run patch while the external `cordis.yml` remains the Bridge model/effort source. Harness loads its own adjacent `.env`. See [`DEEPSEEK_HARNESS_CONNECTION_GUIDE.md`](./DEEPSEEK_HARNESS_CONNECTION_GUIDE.md).
