...
| Antigravity CLI | Required protocol and options advertised by the installed CLI's `--help` | `BridgeAntigravityCLI` probe, stream-json normalization, Plan/Accept Edits, native sandbox, model/effort, continuation and queued steer fixtures | Probe records `--version` for display and determines compatibility from observed capabilities. User registers the real `agy` CLI. CLI authentication and permission policy remain provider-owned. Headless interactive tool approval remains an adapter boundary. |
